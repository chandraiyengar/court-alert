import { SlotInfo } from "./transformer";
...
export class DataValidator {
  static validateSlots(slots: SlotInfo[]): SlotInfo[] {
    ...
    if (validSlots.length !== slots.length) {
      console.warn(
        `⚠️  Filtered out ${slots.length - validSlots.length} invalid slots during validation`
      );
    }
    ...
    if (uniqueSlots.length !== validSlots.length) {
      console.warn(
        `⚠️  Removed ${validSlots.length - uniqueSlots.length} duplicate slots`
      );
    }
  ...
  private static isValidSlot(slot: SlotInfo, index: number): boolean {
    ...
    if (!this.isValidDateFormat(slot.date)) {
      console.warn(`⚠️  Invalid date format at index ${index}: ${slot.date}`);
      return false;
    }

    // Check time format (should be HH:MM:SS)
    if (!this.isValidTimeFormat(slot.time)) {
      console.warn(`⚠️  Invalid time format at index ${index}: ${slot.time}`);
      return false;
    }
    ...
    if (!this.isValidSpaces(slot.spaces)) {
      console.warn(
        `⚠️  Invalid spaces value at index ${index}: ${slot.spaces}`
      );
      return false;
    }
